...
#  - express@4 pinned (avoids path-to-regexp * error in express v5)
#  - SPA fallback uses app.use() instead of app.get('*')
#  - Kannel startup waits increased (8s bearerbox, 6s smsbox) with retry
#  - bcryptjs/multer removed (unused)
# ═══════════════════════════════════════════════════════════════════════════
...
echo ""
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║   NET2APP STANDALONE v2.1   $(date '+%Y-%m-%d')              ║"
...
"
ok "Admin: ${ADMIN_USER}@net2app.local / $ADMIN_PASS"
...
info "Waiting 8s for bearerbox..."
sleep 8
systemctl start kannel-smsbox
info "Waiting 6s for smsbox..."
sleep 6
systemctl is-active kannel-bearerbox && ok "Kannel bearerbox: RUNNING" || { info "Retrying bearerbox..."; systemctl restart kannel-bearerbox; sleep 6; systemctl is-active kannel-bearerbox && ok "Kannel bearerbox: RUNNING" || fail "Kannel bearerbox: FAILED - journalctl -u kannel-bearerbox -n 20"; }
systemctl is-active kannel-smsbox    && ok "Kannel smsbox: RUNNING"    || { info "Retrying smsbox...";    systemctl restart kannel-smsbox;    sleep 6; systemctl is-active kannel-smsbox    && ok "Kannel smsbox: RUNNING"    || fail "Kannel smsbox: FAILED - journalctl -u kannel-smsbox -n 20"; }

header "STEP 7: Kannel Sync Script"
mkdir -p $API_DIR
...
header "STEP 8: Standalone API Server (express@4, NO Base44)"
cd $API_DIR
npm init -y 2>/dev/null | tail -1
# Pin express@4 to avoid path-to-regexp breaking change in express v5
...
const app = express();
// express@4 compatible CORS setup
...
app.get('/api/version', (req, res) => res.json({ version: '2.1.0', standalone: true, base44: false }));
...
app.get('/api/dashboard', async (req, res) => {
  ...
  const [[cc]] = await pool.execute('SELECT COUNT(*) AS cnt FROM clients WHERE status="active"');
  const [[sc]] = await pool.execute('SELECT COUNT(*) AS cnt FROM suppliers WHERE status="active"');
  ...
  res.json({ today: today[0], active_clients: cc[0].cnt, active_suppliers: sc[0].cnt, hourly, recent });
});

app.get('/api/users', adminOnly, async (req, res) => { const [rows]=await pool.execute('SELECT id,name,email,role,status,created_at FROM users ORDER BY created_at DESC'); res.json({ok:true,data:rows}); });
...
app.get('/api/clients', async (req, res) => {
  const {status,search,limit=100}=req.query; let q='SELECT * FROM clients'; const p=[],w=[];
  if (status){w.push('status=?');p.push(status);} if (search){w.push('(name LIKE ? OR email LIKE ?)');p.push(`%${search}%`,`%${search}%`);}
  if (w.length) q+=' WHERE '+w.join(' AND '); q+=' ORDER BY created_at DESC LIMIT ?'; p.push(parseInt(limit));
  const [rows]=await pool.execute(q,p); res.json({ok:true,data:rows});
});
app.get('/api/clients/:id', async (req, res) => { const [[r]]=await pool.execute('SELECT * FROM clients WHERE id=?',[req.params.id]); if(!r)return res.status(404).json({error:'Not found'}); res.json(r); });
...
app.get('/api/suppliers', async (req, res) => {
  const {category,status}=req.query; let q='SELECT * FROM suppliers'; const p=[],w=[];
  if (category){w.push('category=?');p.push(category);} if (status){w.push('status=?');p.push(status);}
  if (w.length) q+=' WHERE '+w.join(' AND '); q+=' ORDER BY priority ASC,created_at DESC';
  const [rows]=await pool.execute(q,p); res.json({ok:true,data:rows});
});
...
app.get('/api/rates', async (req, res) => {
  ...
  if (entity_id){q+=' AND entity_id=?';p.push(entity_id);} if (type){q+=' AND type=?';p.push(type);} q+=' ORDER BY mcc,mnc';
  const [rows]=await pool.execute(q,p); res.json({ok:true,data:rows});
});
...
app.put('/api/rates/:id', async (req, res) => { const d=req.body; await pool.execute('UPDATE rates SET rate=?,currency=?,status=? WHERE id=?',[d.rate||0,d.currency||'USD',d.status||'active',req.params.id]); res.json({ok:true}); });
...
app.get('/api/sms-logs', async (req, res) => {
  ...
  const [[{cnt}]]=await pool.execute('SELECT COUNT(*) AS cnt FROM sms_log'+(w.length?' WHERE '+w.join(' AND '):''),p.slice(0,-2));
...
app.post('/api/send', async (req, res) => {
  ...
  if (!sid) return res.status(400).json({error:'No active route for client'});
  ...
  if (supplier.connection_type==='SMPP') {
    ...
    await pool.execute('UPDATE sms_log SET status=?,dest_msg_id=? WHERE message_id=?',[r.ok&&(body.includes('Accepted')||body.includes('0:'))?'sent':'failed',body.substring(0,64),msgId]);
    return res.json({ok:r.ok,message_id:msgId,kannel_response:body});
  }
  if (supplier.connection_type==='HTTP'&&supplier.http_url) {
    ...
    const r=await fetch(supplier.http_method==='POST'?supplier.http_url:`${supplier.http_url}?${qs}`,{
      method:supplier.http_method||'GET',headers:{'Content-Type':'application/x-www-form-urlencoded'},
      body:supplier.http_method==='POST'?qs:undefined,signal:AbortSignal.timeout(15000)
    });
...
app.get('/api/kannel/status', async (req, res) => {
  try {
    ...
    const text=await r.text(); res.json({ok:true,up:text.includes('bearerbox')||text.includes('uptime'),raw:text.substring(0,800)});
...
app.get('/api/billing/summary', async (req, res) => {
  const {from,to,client_id}=req.query; let q='SELECT * FROM billing_summary'; const p=[],w=[];
...
app.post('/api/invoices', adminOnly, async (req, res) => {
  ...
  await pool.execute('INSERT INTO invoices (id,client_id,client_name,invoice_number,period_start,period_end,total_sms,amount,currency,status,notes) VALUES (?,?,?,?,?,?,?,?,?,?,?)',[id,d.client_id,d.client_name||'',inv_num,d.period_start||null,d.period_end||null,d.total_sms||0,d.amount||0,d.currency||'USD',d.status||'draft',d.notes||'']);
  res.json({ok:true,id,invoice_number:inv_num});
});
app.put('/api/invoices/:id', adminOnly, async (req, res) => { await pool.execute('UPDATE invoices SET status=?,notes=? WHERE id=?',[req.body.status||'draft',req.body.notes||'',req.params.id]); res.json({ok:true}); });
...
app.post('/api/settings', adminOnly, async (req, res) => {
  ...
  const id=uuid();
  await pool.execute('INSERT INTO system_settings (id,setting_key,setting_value,category,description) VALUES (?,?,?,?,?) ON DUPLICATE KEY UPDATE setting_value=VALUES(setting_value),updated_at=NOW()',[id,setting_key,setting_value||'',category||'system',description||'']);
  res.json({ok:true});
});
...
app.post('/api/alert-rules', adminOnly, async (req, res) => {
  const d=req.body; const id=uuid();
  await pool.execute('INSERT INTO alert_rules (id,name,alert_type,threshold,window_minutes,min_messages,notify_email,client_id,supplier_id,severity,is_active,cooldown_minutes) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)',[id,d.name,d.alert_type,d.threshold||0,d.window_minutes||60,d.min_messages||10,d.notify_email||'',d.client_id||null,d.supplier_id||null,d.severity||'warning',d.is_active!==false?1:0,d.cooldown_minutes||60]);
  res.json({ok:true,id});
});
app.put('/api/alert-rules/:id', adminOnly, async (req, res) => {
  const d=req.body; await pool.execute('UPDATE alert_rules SET name=?,threshold=?,notify_email=?,severity=?,is_active=? WHERE id=?',[d.name,d.threshold||0,d.notify_email||'',d.severity||'warning',d.is_active?1:0,req.params.id]); res.json({ok:true});
});
...
app.get('/api/mccmnc', async (req, res) => {
  ...
  q+=' ORDER BY mcc,mnc LIMIT 200'; const [rows]=await pool.execute(q,p); res.json({ok:true,data:rows});
});

// SPA FALLBACK — use app.use() not app.get('*') to avoid path-to-regexp error in express v5+
app.use((req, res) => {
  const index = path.join(WEBROOT, 'index.html');
  if (fs.existsSync(index)) res.sendFile(index);
  else res.json({ service: 'Net2app Standalone API', version: '2.1.0', standalone: true, login: 'POST /api/auth/login' });
});
...
app.listen(PORT,'0.0.0.0',async()=>{
  console.log(`Net2app Standalone API v2.1 on port ${PORT}`);
  console.log(`express@4 — NO path-to-regexp issues`);
  console.log(`Admin: admin@net2app.local / Admin@2025!`);
...
info "Waiting 5s for API..."
sleep 5
curl -s http://127.0.0.1:5000/health | grep -q '"ok":true' && ok "Standalone API: RUNNING on :5000" || { info "Waiting 5 more seconds..."; sleep 5; curl -s http://127.0.0.1:5000/health | grep -q '"ok":true' && ok "API: RUNNING" || fail "API FAILED — run: pm2 logs net2app-api"; }
...
ufw allow 9095:9200/tcp; ufw deny 3306
...
echo ""
echo "╔══════════════════════════════════════════════════════════════════╗"
echo "║  NET2APP STANDALONE v2.1 — DEPLOYMENT COMPLETE               ║"
echo "╠══════════════════════════════════════════════════════════════════╣"
...
echo "║  API:   POST /api/auth/login → JWT token                     ║"
echo "╠══════════════════════════════════════════════════════════════════╣"
echo "║  ✅ express@4 — no path-to-regexp errors                      ║"
echo "║  ✅ NO Base44 — 100% self-hosted                              ║"
echo "║  ✅ JWT auth + full CRUD API (30+ endpoints)                  ║"
echo "║  ✅ SMS via Kannel SMPP or HTTP suppliers                     ║"
echo "║  ✅ DLR at /api/dlr | Kannel sync: gen-kannel-conf.sh         ║"
echo "╚══════════════════════════════════════════════════════════════════╝"
